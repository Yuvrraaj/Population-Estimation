...
class ModernYOLOApp:
    def __init__(self, root):
        ...
        self.use_dhm = tk.BooleanVar(value=False)
        self.file_path = None
        self.setup_window()
        self.create_styles()
        self.create_widgets()
    ...
    def create_control_panel(self, parent):
        ...
        self.use_dhm_checkbox = tk.Checkbutton(input_frame, text="Use DSM/DTM for floor estimation", variable=self.use_dhm, bg="#1a1f26", fg="white", selectcolor="#1a1f26", activebackground="#1a1f26")
        self.use_dhm_checkbox.pack(anchor=tk.W, pady=5)
    ...
    def analyze_image(self, file_path):
        try:
            ...
            use_dhm = self.use_dhm.get()

            dsm = dtm = None
            if use_dhm:
                try:
                    import rasterio
                    dsm = rasterio.open("C:/Users/gpu18/Desktop/Current/DSM.tif")
                    dtm = rasterio.open("C:/Users/gpu18/Desktop/Current/DTM.tif")
                except:
                    self.status_var.set("DSM/DTM not found. Falling back to area-only mode.")
                    use_dhm = False
                ...
                if cls == 0:
                    ...
                    if use_dhm and dsm and dtm:
                        try:
                            ...
                            avg_height = np.mean(height_map) if height_map.size > 0 else 3
                            floors = max(1, round(avg_height / 3))
                        except:
                            floors = 1
                    estimated_population += self.people_per_building.get() * multiplier * floors
                elif cls == 1:
                    non_residential_count += 1
                cv2.rectangle(processed_image, (x1, y1), (x2, y2), colors.get(cls, (255, 255, 255)), 2)
