...
class BuildingCountPopulationEstimator:
    ...
    def create_tiles_from_tif_with_coordinates(self, tif_path, output_dir, tile_area_sqm):
        """Create tiles based on AREA (square meters) by iterating over world coordinates"""
        import os
        import numpy as np
        import rasterio
        from rasterio.windows import Window, from_bounds
        ...
        try:
            with rasterio.open(tif_path) as src:
                ...
                # CRITICAL: Get GSD, but ONLY for metadata and area calculation.
                # We will NOT use it for the tiling grid.
                pixel_width_meters = abs(original_transform[0])
                pixel_height_meters = abs(original_transform[4])
                gsd = (pixel_width_meters + pixel_height_meters) / 2.0
                
                print(f"\n{'='*70}")
                print(f"COORDINATE-BASED TILE GENERATION (Pure Area)")
                print(f"{'='*70}")
                ...
                print(f"Average GSD: {gsd:.4f} meters/pixel (for metadata only)")
                
                # Calculate tile side length in METERS
                ...
                # --- NEW LOGIC: Iterate over WORLD COORDINATES ---
                # Create coordinate grids in meters
                x_coords = np.arange(bounds.left, bounds.right, tile_side_meters)
                y_coords = np.arange(bounds.top, bounds.bottom, -tile_side_meters) # Y goes from Top to Bottom

                tile_count = 0
                for y_top in y_coords:
                    for x_left in x_coords:
                        
                        # Define tile bounds in world coordinates
                        x_right = x_left + tile_side_meters
                        y_bottom = y_top - tile_side_meters
                        
                        # Clip bounds to the actual image extent
                        tile_x_left = x_left
                        tile_y_bottom = max(y_bottom, bounds.bottom)
                        tile_x_right = min(x_right, bounds.right)
                        tile_y_top = y_top
                        
                        # Calculate actual tile dimensions in meters
                        actual_width_m = tile_x_right - tile_x_left
                        actual_height_m = tile_y_top - tile_y_bottom
                        actual_tile_area = actual_width_m * actual_height_m

                        # Skip tiles that are too small (less than 25% of target side)
                        min_acceptable_side_m = tile_side_meters / 4.0
                        if actual_width_m < min_acceptable_side_m or actual_height_m < min_acceptable_side_m:
                            continue

                        # Get the pixel window corresponding to these world coordinates
                        tile_geo_bounds = (tile_x_left, tile_y_bottom, tile_x_right, tile_y_top)
                        try:
                            window = from_bounds(*tile_geo_bounds, src.transform)
                            # Sanitize window
                            ...
                            print(f"Skipping tile at {tile_geo_bounds}, error: {e}")
                            continue

                        # Get the transform for this specific tile
                        tile_transform = rasterio.windows.transform(window, original_transform)
                        
                        # Create tile filename with world coordinates
                        world_x = int(tile_x_left)
                        world_y = int(tile_y_top)
                        tile_filename = f"tile_X{world_x}_Y{world_y}_area{int(actual_tile_area)}sqm.tif"
                        tile_path = os.path.join(output_dir, tile_filename)

                        # Read tile data from the calculated window
                        ...
                        tile_metadata = {
                            'path': tile_path,
                            'row': None, # Pixel row is no longer relevant
                            'col': None, # Pixel col is no longer relevant
                            'transform': tile_transform,
                            'crs': original_crs,
                            'bounds': tile_geo_bounds,
                            'shape': (window.height, window.width),
                            'area_sqm': actual_tile_area,
                            'gsd': gsd  # Store GSD for downstream analysis
                        }
                        tile_metadata_list.append(tile_metadata)
                        tile_count += 1

                        if tile_count % 10 == 0:
                            print(f"Created {tile_count} coordinate-based tiles...")
                
                print(f"Successfully created {len(tile_metadata_list)} coordinate-based tiles")
                print(f"Average tile area: {np.mean([t['area_sqm'] for t in tile_metadata_list]):.2f} m²")

        except Exception as e:
            print(f"Error creating coordinate-based tiles: {e}")
            raise
    ...
    def _update_processed_label(self, annotated_path, generation):
        """
        Safely update processed_label with annotated image only if generation matches current.
        Usage: increment self._annotated_generation before scheduling background annotation work,
        and pass the generation value when calling this method via self.root.after(...).
        """
        try:
            # If there's no generation stored yet, default to 0
            current_gen = getattr(self, "_annotated_generation", 0)
            # Reject stale updates
            if generation != current_gen:
                return

            # If no path or file doesn't exist -> clear display
            if not annotated_path or not os.path.exists(annotated_path):
                try:
                    self.processed_label.config(image='', text="No result image")
                    self.processed_label.image = None
                except Exception:
                    pass
                return

            # Remove previous annotated temp file if it's different from the new one
            prev = getattr(self, 'last_annotated_path', None)
            if prev and prev != annotated_path:
                try:
                    # Only delete if it looks like a temp/derived file (non-original). Adjust if needed.
            ...
            # Load and prepare image for Tkinter
            from PIL import Image, ImageTk
            image = Image.open(annotated_path)
            # Keep a reasonable thumbnail to avoid huge memory usage in UI
            image.thumbnail((400, 200), Image.Resampling.LANCZOS)
            image_tk = ImageTk.PhotoImage(image)

            # Clear any previous image reference from the label first
            try:
            ...
            # Set the new image and save reference so Tkinter doesn't GC it
            self.processed_label.config(image=image_tk, text="")
            self.processed_label.image = image_tk

            # Save last path so we can delete temp files later if needed
            self.last_annotated_path = annotated_path

        except Exception as e:
            # Keep error handling non-fatal - show an informative label text
            try:
                self.processed_label.config(text=f"❌ Error updating result image:\n{str(e)}")
                self.processed_label.image = None
            except Exception:
                pass
            print(f"Error in _update_processed_label: {e}")
    ...
    def _utm_epsg_from_bounds(self, crs, bounds):
        """Pick a metric CRS (UTM) from raster bounds. Works even if source is EPSG:4326."""
        try:
            import math
            # If geographic, bounds are (lon/lat); if projected, try to get center via transform to WGS84
            if crs and crs.is_geographic:
                lon = (bounds.left + bounds.right) / 2.0
                lat = (bounds.top + bounds.bottom) / 2.0
            else:
                # Reproject center to WGS84 to find UTM zone
                from rasterio.warp import transform_bounds
                wgs_bounds = transform_bounds(crs, "EPSG:4326",
                                            bounds.left, bounds.bottom, bounds.right, bounds.top,
                                            densify_pts=21)
                lon = (wgs_bounds[0] + wgs_bounds[2]) / 2.0
                lat = (wgs_bounds[1] + wgs_bounds[3]) / 2.0
            ...
            # Fallback: Web Mercator is metric; not ideal for area but safe fallback
            return "EPSG:3857"

    def _pixel_size_meters(self, src):
        """Return approximate (res_x_m, res_y_m) for an open rasterio dataset."""
        import math
        res_x, res_y = src.res
        if src.crs and src.crs.is_geographic:
            # degrees -> meters (approx) using latitude scale
            ...
            return abs(res_x) * abs(m_per_deg_lon), abs(res_y) * abs(m_per_deg_lat)
        else:
            return abs(res_x), abs(res_y)  # already meters (projected CRS)

    def _reproject_resample(self, in_path, out_path, dst_crs, dst_res):
        """
        Reproject+resample to (dst_crs, dst_res[m, m]) using bilinear for imagery; preserves bands, dtype.
        """
        import rasterio
        from rasterio.warp import calculate_default_transform, reproject, Resampling
    ...
    def _tile_area_m(self, base_ds, tile_area_sqm, out_dir, prefix):
        """
        Cut pure-area tiles from base_ds (already metric CRS), write tiles, and
        return metadata list. This is the ONLY place tiles are cut.
        """
        ...
        # side in meters
        side_m = math.sqrt(tile_area_sqm)
        left, bottom, right, top = base_ds.bounds
        # walk grid in meters
        xs = []
        x = left
        while x < right:
        ...
        for r in range(len(ys)-1):
            ...
            for c in range(len(xs)-1):
                x_left = xs[c]
                x_right = xs[c+1]
                # window in pixels for this geog box
                w = from_bounds(x_left, y_bottom, x_right, y_top, transform=base_ds.transform)
                # Make sure window is within the dataset; allow boundless read with fill
                data = base_ds.read(window=w, boundless=True, fill_value=0)
                # If completely empty, skip
                if data.size == 0 or (np.all(data == 0)):
                    continue

                # output geotransform for this tile
                t_transform = win_transform(w, base_ds.transform)

                # update profile for tile dims
                tile_profile = meta.copy()
                tile_profile.update({
                    "width": data.shape[2],
                    "height": data.shape[1],
                ...
                fname = f"{prefix}_X{int(round(x_left))}to{int(round(x_right))}_Y{int(round(y_bottom))}to{int(round(y_top))}_A{int(round(tile_area_sqm))}.tif"
                fpath = os.path.join(out_dir, fname)

                with rasterio.open(fpath, "w", **tile_profile) as dst:
                    dst.write(data)
                ...
                tiles.append({
                    ...
                    "row": r,
                    "col": c,
                    "geographic_bounds": (x_left, y_bottom, x_right, y_top),
                    "transform": t_transform,
                    "crs": base_ds.crs,
                    "area_sqm": (x_right - x_left) * (y_top - y_bottom)
                })

        return tiles, side_m

    def _collect_inputs_for_resample(self):
        """
        Build list of rasters to align. Optical is mandatory; include DSM/DTM if set.
        Return list of (label, path).
        """
        items = [("optical", self.file_path)]
        # if your app5.py uses these attributes; if not, this will be just optical.
        if getattr(self, "dhm_path", None):
            items.append(("dsm", self.dhm_path))
        if getattr(self, "dtm_path", None):
            items.append(("dtm", self.dtm_path))
        return items
    ...
    def _generate_tiles_thread(self):
        """
        FULL pipeline:
        1) Choose target metric CRS (UTM from optical)
        2) Compute common pixel size in meters (min across inputs)
        3) Reproject+resample all inputs to (CRS, pixel)
        4) Tile by pure area in meters from OPTICAL-aligned raster
        """
        import os
        import numpy as np
        import tempfile
        ...
        try:
            tile_area = float(self.tile_area_sqm.get())
            if tile_area <= 0:
                raise ValueError("Tile area must be positive")

            # -------- (1) inputs & target CRS --------
            inputs = self._collect_inputs_for_resample()
            if not inputs or not inputs[0][1]:
                raise RuntimeError("No optical image selected")

            optical_path = inputs[0][1]
            with rasterio.open(optical_path) as src_opt:
                target_crs = self._utm_epsg_from_bounds(src_opt.crs, src_opt.bounds)

            # -------- (2) common pixel size (meters) --------
            # estimate native GSD (m) for each input, pick the smallest
            gsds = []
            for _, p in inputs:
                with rasterio.open(p) as ds:
                    rx_m, ry_m = self._pixel_size_meters(ds)
                    gsds.append(min(rx_m, ry_m))
            target_res_m = float(np.nanmin(gsds))

            # Safety clamps (avoid insane sub-decimeter if metadata odd)
            if target_res_m <= 0 or target_res_m != target_res_m:
                target_res_m = 0.5  # default
            target_res_m = max(0.2, min(target_res_m, 5.0))  # clamp 0.2..5 m

            # -------- (3) reproject+resample all inputs --------
            res_dir = os.path.join(self.tiles_save_path, "_resampled")
            os.makedirs(res_dir, exist_ok=True)

            aligned_paths = {}
            for label, p in inputs:
                out_p = os.path.join(res_dir, f"{label}_aligned.tif")
                self._reproject_resample(p, out_p, target_crs, (target_res_m, target_res_m))
                aligned_paths[label] = out_p

            # -------- (4) area tiling from optical-aligned --------
            with rasterio.open(aligned_paths["optical"]) as base_ds:
                tiles, side_m = self._tile_area_m(base_ds, tile_area, self.tiles_save_path, prefix="tile")
            ...
            def _ok():
                ...
                self.tiles_status.config(
                    text=f"✅ Generated {n} tiles @ {side_m:.2f} m side; common GSD {target_res_m:.3f} m; CRS {target_crs}."
                )
                self.generate_tiles_btn.config(state='normal', text="🔄 Generate Area-Based Tiles")
                self.show_progress(False)
                self.update_status(f"Area-based tiles generated: {n}", "✅")
                self.update_validation_status()

            self.root.after(0, _ok)

        except Exception as err:
            def _fail(msg=str(err)):
                # Ensure 'err' is captured safely
                self.generate_tiles_btn.config(state='normal', text="🔄 Generate Area-Based Tiles")
                self.show_progress(False)
                self.update_status("Area-based tile generation failed", "❌")
                try:
                    messagebox.showerror("Area-Based Tile Generation Error", msg)
                except Exception:
                    pass
    ...
    def analyze_single_image(self, image_path):
        ...
        try:
            ...
            # --- NEW: create annotated image and schedule GUI update safely ---
            annotated_path = None
            if yolo_results and len(yolo_results) > 0:
                try:
                    result = yolo_results[0]
                    annotated_image = result.plot()  # BGR numpy array from ultralytics
                    import cv2, tempfile
                    # Use a unique temp file (delete=False) and write it
                    tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".png")
                    annotated_path = tmp.name
                    tmp.close()
                    # write BGR array directly
                    cv2.imwrite(annotated_path, annotated_image)
                except Exception as e:
                    print(f"Error creating annotated image: {e}")
                    annotated_path = None

            # --- schedule a single main-thread update so the correct image sticks ---
            try:
                # increment generation to mark this as the latest analysis
                self._annotated_generation = getattr(self, "_annotated_generation", 0) + 1
                gen = self._annotated_generation

                # schedule update on main thread (atomic, will ignore stale updates)
                if annotated_path and os.path.exists(annotated_path):
                    self.root.after(0, lambda p=annotated_path, g=gen: self._update_processed_label(p, g))
                else:
                    # if something went wrong creating the annotated image, clear label
                    self.root.after(0, lambda g=gen: self._update_processed_label(None, g))
            except Exception as e:
                print(f"Error scheduling processed image update: {e}")
    ...
    def update_processed_image_display(self, results):
        ...
        try:
            if self.is_tif_file and self.tile_metadata_list:
                ...
                # For single images, DO NOTHING.
                # The correct image was already generated and scheduled for display
                # by the analyze_single_image() thread using _update_processed_label.
                # Calling display_processed_single_image(results) here would
                # overwrite the correct image with a re-generated one.
                pass
        except Exception as e:
