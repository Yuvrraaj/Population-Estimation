...
class ModernYOLOApp:
    ...
    def analyze_image(self, file_path):
        try:
            ...
            # Load DSM and DTM (update these paths to your actual DSM/DTM files)
            dsm_path = "DSM.tif"
            dtm_path = "DTM.tif"
            try:
                ...
                messagebox.showerror("DSM/DTM Load Error", f"Failed to load DSM/DTM files: {str(e)}")
                return
            ...
            for box in results.boxes:
                ...
                if cls == 0:
                    residential_count += 1
                    bbox = (x1, y1, x2, y2)
                    avg_height = self.get_building_height(dsm, dtm, bbox)
                    floors = max(1, round(avg_height / 3.0))
                    estimated_population += floors * people_per_floor
    ...
    def get_building_height(self, dsm, dtm, bbox):
        try:
            bounds = rasterio.windows.from_bounds(*bbox, transform=dsm.transform)
            dsm_crop = dsm.read(1, window=bounds)
            dtm_crop = dtm.read(1, window=bounds)
            height = dsm_crop - dtm_crop
            height = height[height > 0]
            return np.mean(height) if height.size > 0 else 0
        except Exception as e:
            print("Error extracting height:", e)
            return 0
